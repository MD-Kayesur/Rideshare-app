        ...
        <View style={tw`flex-1 bg-white`}>
            ...
            <SafeAreaView style={tw`flex-1`}>
                ...
                <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={tw`pb-32 px-6`}>
                    ...
                    {/* Car Summary */}
                    <View style={tw`bg-[#E6F7F1]/30 border border-[#10B981]/10 rounded-2xl p-4 flex-row items-center mb-8`}>
                        <View style={tw`flex-1`}>
                            <Text style={tw`text-lg font-bold text-gray-800 mb-1`}>{name || 'Mustang Shelby GT'}</Text>
                    ...
                    {paymentMethods.map((method) => (
                        <Pressable
                            ...
                            style={tw`flex-row items-center bg-white border ${selectedPayment === method.id ? 'border-[#10B981]' : 'border-gray-100'} rounded-xl p-4 mb-3 shadow-sm`}
                        >
                            <View style={tw`w-12 h-10 items-center justify-center mr-4`}>
                                <FontAwesome name={method.icon as any} size={28} color={method.type === 'visa' ? '#1A1F71' : method.type === 'mastercard' ? '#EB001B' : '#0070BA'} />
                            </View>
                            <View style={tw`flex-1`}>
                                <Text style={tw`text-gray-700 font-bold`}>{method.label}</Text>
                                <Text style={tw`text-gray-400 text-xs`}>{method.expires}</Text>
                            </View>
                        </Pressable>
                    ))}
                </ScrollView>
                ...
                <View style={tw`absolute bottom-0 left-0 right-0 bg-white px-6 py-6 border-t border-gray-100`}>
                    ...
                    >
                        <Text style={tw`text-white font-bold text-lg`}>Confirm Booking</Text>
                    </Pressable>
                </View>
            </SafeAreaView>
        </View>
